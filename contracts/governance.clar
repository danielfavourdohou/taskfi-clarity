...
(define-map voter-power { proposal-id: uint, voter: principal } uint)
...
(define-public (execute-proposal (proposal-id uint))
  (let ((proposal (unwrap! (map-get? proposals proposal-id) ERR_PROPOSAL_NOT_FOUND))
    ...
    (begin
      (asserts! (var-get governance-active) ERR_UNAUTHORIZED)
      (asserts! (>= current-block (get execution-available-at proposal)) ERR_VOTING_ACTIVE)
      (asserts! (not (get executed proposal)) ERR_PROPOSAL_EXECUTED)
      (asserts! (not (get cancelled proposal)) ERR_PROPOSAL_NOT_FOUND)

      ;; Check if proposal passed
      (let ((total-reputation (contract-call? .reputation get-total-reputation))
        ...
        (begin
          (asserts! quorum-met ERR_INVALID_INPUT)
          (asserts! approval-met ERR_INVALID_INPUT)
          ...
          (if (is-eq (get proposal-type proposal) PROPOSAL_TYPE_PARAMETER)
            ...
            (ok true)) ;; Other proposal types would be handled here

          (ok true))))))

;; Execute parameter changes
(define-private (execute-parameter-change (proposal {proposer: principal, title: (string-ascii 64), description: (string-ascii 256), proposal-type: uint, target-parameter: (optional (string-ascii 32)), new-value: (optional uint), created-at: uint, voting-ends-at: uint, execution-available-at: uint, yes-votes: uint, no-votes: uint, total-voting-power: uint, executed: bool, cancelled: bool}))
...
(define-read-only (get-proposal-status (proposal-id uint))
  (match (map-get? proposals proposal-id)
    proposal (let ((current-block stacks-block-height))
              {
                voting-active: (and (<= current-block (get voting-ends-at proposal))
                                   (not (get executed proposal))
                                   (not (get cancelled proposal))),
                can-execute: (and (>= current-block (get execution-available-at proposal))
                                 (not (get executed proposal))
                                 (not (get cancelled proposal))),
